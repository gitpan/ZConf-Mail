...
sub new {
	...
	$self->{legal}{imap}=['user', 'pass', 'useSSL','deliverTo',
						  'fetchable', 'inbox', 'server', 'port'];
	$self->{legal}{mbox}=['mbox', 'deliverTo', 'fetchable'];
	$self->{legal}{smtp}=['user', 'pass', 'auth', 'useSSL',
						  'server', 'port', 'name', 'from',
						  'name', 'timeout'];
	$self->{legal}{maildir}=['maildir','deliverTo', 'fetchable'];

	#this defines the required values for later use
	#currently the same as for 
	$self->{required}{pop3}=['user', 'pass', 'auth', 'useSSL',
						  'deliverTo', 'fetchable', 'server', 'port'];
	$self->{required}{imap}=['user', 'pass', 'useSSL', 'deliverTo',
						  'fetchable', 'inbox', 'server', 'port'];
...
sub delAccount{
	...
	my @deleted=$self->{zconf}->regexVarSearch('mail', '^accounts/'.$account.'/');
...
sub fetchIMAP{
	...
	if ($self->{error}) {
		warn('ZConf-Mail fetchIMAP:22: Failed to connect to the maildir.');
		return undef;
	}
...
Gets a array of the various SMTP accounts.
...
sub getAccounts{
	...
	#each match is then the at every '/'...
	#the split is then dumped into %accountsH as a key of the 1 and 2 positions...
	#so for 'accounts/pop3/monkey/...', it would be 'pop3/monkey'...
	#
	...
	while (defined($matched[$matchedInt])) {
		my @split=split(/\//, $matched[$matchedInt],3);
...
sub send{
	...
	if (!$smtp->dataend) {
		warn('ZConf-Mail send:29: Failed to send the data.');
		$self->{error}=29;
